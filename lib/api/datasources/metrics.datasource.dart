...
class MetricsDatasourceImpl implements MetricsDatasource {
  ...
  Future<MetricsModel> fetchMetrics() async {
    try {
      ...
      List<dynamic> expensesAmounts = expenses.map((e) => e.amount).toList();
      List<dynamic> revenuesAmounts = revenues.map((e) => e.amount).toList();

      final totalExpenses = expensesAmounts.fold(0.0, (previousValue, e) => previousValue + int.parse(e));
      final totalRevenues = revenuesAmounts.fold(0.0, (previousValue, e) => previousValue + int.parse(e));
